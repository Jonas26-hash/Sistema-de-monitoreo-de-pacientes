...
public class PreRegistroResource {
    ...
    public Response preRegistro(@Valid PreRegistroRequest request) {
        ...
        if (request.dni != null && !request.dni.isEmpty()
            && Paciente.find("dni", request.dni).firstResult() != null) {
            return Response.status(Response.Status.CONFLICT)
                .entity("{\"error\":\"DNI ya registrado\"}")
                .build();
        }
        ...
        return Response.ok("{\"mensaje\":\"Código de verificación enviado a tu correo\"}").build();
    }
    ...
    public Response preRegistroPersonal(@Valid PreRegistroRequest request) {
        ...
        if (request.rolSolicitado == Rol.PACIENTE) {
            return Response.status(Response.Status.BAD_REQUEST)
                .entity("{\"error\":\"Usa pre-registro para pacientes\"}")
                .build();
        }
        ...
        return Response.ok("{\"mensaje\":\"Código de verificación enviado al correo del empleado\"}").build();
    }
